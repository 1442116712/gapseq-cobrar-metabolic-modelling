...
# ---------- Configuration ----------
xlsx_path <- "C:/Users/CFL/OneDrive - Queen's University Belfast/PhD_onedrive/Manuscript4_gapseq/others/M4_Supplement.xlsx"
...
fig1 <- (p_a | p_b) / (p_c | p_d) +
  plot_layout(heights = c(1, 1), widths = c(1, 1.3)) &
  theme(plot.margin = margin(t = 20, r = 25, b = 20, l = 25))

out_png <- file.path(out_dir, "Fig1.png")
ggsave(out_png, fig1, width = 17.5, height = 10, dpi = 300)
cat(sprintf(">>> Wrote %s\n", out_png))
