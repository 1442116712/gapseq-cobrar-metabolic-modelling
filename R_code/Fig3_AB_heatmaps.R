...
p_a <- ggplot(aux_long_A, aes(x = compound, y = organism, fill = category)) +
  ...
  labs(title = "Auxotrophies across all metabolic models",
       x = NULL, y = NULL) +
  ...
  theme(
    ...
    axis.text.y         = element_text(face = "bold", colour = "grey15", size = 9.5),
...
panel_B <- p_strip_B / p_heat_B + plot_layout(heights = c(0.04, 1))
