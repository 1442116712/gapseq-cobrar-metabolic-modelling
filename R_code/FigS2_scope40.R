# =============================================================================
# SuppFig_PCoA_40_Gower.R
# Manuscript 4 - Supplementary niche partitioning analysis (variant 2)
# PCoA trained on 40 metabolic models (34 pan-Draft + 6 reference);
# 2 ARG-MAGs projected passively via Gower (1968) out-of-sample formula.
# References act as cultured anchors in the niche space; ARG-MAGs are
# the only out-of-sample test points.
# =============================================================================
...
# 40 training matrix (34 pan + 6 ref): substrates
sub_train_data <- bind_rows(
  sub_pan %>% select(organism, compound, Net_Growth),
  sub_ref %>% select(organism, compound, Net_Growth)
)
train_orgs <- c(pan_orgs, keep_ref)
sub_mat_train <- build_mat(sub_train_data, "Net_Growth", train_orgs)

# 2 ARG-MAG matrix for substrate (project onto training space)
sub_mat_arg <- build_mat(sub_arg, "Net_Growth", keep_argmag)
miss <- setdiff(colnames(sub_mat_train), colnames(sub_mat_arg))
if (length(miss) > 0) {
  add <- matrix(0, nrow = nrow(sub_mat_arg), ncol = length(miss),
                dimnames = list(rownames(sub_mat_arg), miss))
  sub_mat_arg <- cbind(sub_mat_arg, add)
}
sub_mat_arg <- sub_mat_arg[, colnames(sub_mat_train), drop = FALSE]

# 40 training matrix: auxotrophies (binary)
aux_train_data <- bind_rows(
  aux_pan %>% select(organism, compound),
  aux_ref %>% select(organism, compound)
) %>% mutate(present = 1L)
aux_mat_train <- build_mat(aux_train_data, "present", train_orgs,
                            fill = 0L, binary = TRUE)

aux_arg2 <- aux_arg %>% mutate(present = 1L)
aux_mat_arg <- build_mat(aux_arg2, "present", keep_argmag,
                          fill = 0L, binary = TRUE)
miss <- setdiff(colnames(aux_mat_train), colnames(aux_mat_arg))
if (length(miss) > 0) {
  add <- matrix(0L, nrow = nrow(aux_mat_arg), ncol = length(miss),
                dimnames = list(rownames(aux_mat_arg), miss))
  aux_mat_arg <- cbind(aux_mat_arg, add)
}
aux_mat_arg <- aux_mat_arg[, colnames(aux_mat_train), drop = FALSE]
...
cat(sprintf("ARG-MAG set:  %d organisms (projected)\n",
            nrow(sub_mat_arg)))
...
# Training coordinates (both pan-Draft AND reference)
...
# ---- Gower (1968) out-of-sample projection for 2 ARG-MAGs --------------------
...
arg_pcs_sub <- project_gower(sub_mat_arg, sub_mat_train, method = "bray") %>%
  left_join(arg_meta, by = "organism")

arg_pcs_aux <- project_gower(aux_mat_arg, aux_mat_train,
                             method = "jaccard", binary = TRUE) %>%
  left_join(arg_meta, by = "organism")

# ---- PERMANOVA + betadisper on 40 training set -------------------------------
...
cat("\n=== Supplementary (40-train + 2 ARG-MAG projection) ===\n")
...
# ---- Plot helper -------------------------------------------------------------
plot_pcoa <- function(train_pcs, arg_pcs, var_pct, title,
                      perm_F, perm_p, disp_p) {
  pan_pts <- train_pcs %>% filter(role == "pan")
  ref_pts <- train_pcs %>% filter(role %in% c("ref_gapseq","ref_rumen")) %>%
  ...
  ggplot() +
    ...
    geom_point(data = pan_pts,
               ...
               alpha = 0.85) +
    # Reference training points (real coordinates, shape distinguishes role)
    ...
    # ARG-MAG: the only projected (out-of-sample) points
    ...
    scale_shape_manual(values = c(ref_gapseq = 24, ref_rumen = 25),
                       ...
                       name = "Role") +
    labs(title    = title,
         subtitle = sprintf("PERMANOVA phylum (40 training): F = %.2f, p = %.3f; betadisper p = %.3f",
                            perm_F, perm_p, disp_p),
...
p_sub <- plot_pcoa(train_pcs_sub, arg_pcs_sub, var_sub,
                   "A   Substrate utilisation (40 training, ARG-MAGs projected)",
                   perm_sub$F[1], perm_sub$`Pr(>F)`[1],
                   disp_sub_test$tab$`Pr(>F)`[1])
p_aux <- plot_pcoa(train_pcs_aux, arg_pcs_aux, var_aux,
                   "B   Auxotrophy patterns (40 training, ARG-MAGs projected)",
...
ggsave(file.path(out_dir, "SuppSupplementary Figure S2 (40 Gower).png"),
...
write.table(bind_rows(
  ...
  arg_pcs_sub   %>% mutate(matrix = "substrate",  projected = "Gower_1968"),
  train_pcs_aux %>% mutate(matrix = "auxotrophy", projected = "training"),
  arg_pcs_aux   %>% mutate(matrix = "auxotrophy", projected = "Gower_1968")
), file = file.path(out_dir, "SuppFig_PCoA_40_Gower_coords.tsv"),
