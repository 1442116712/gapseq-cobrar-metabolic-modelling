...
m2_csv     <- Sys.getenv("M2_CSV",
  "/users/40335635/sharedscratch/EggNOG/manuscript_4/single_genome/M2_no_rumen.csv")
...
# ---------------------- 9. M2-no-rumen defined-medium test ----------------------
# Wet-lab observation: DSM 11370 grows on full M2 (which contains 20 % v/v
# clarified rumen fluid, CRF) but not on FAB. We constructed a defined-medium
# reconstruction of M2 with the CRF removed (M2_no_rumen.csv) to test whether
# the chemically-defined components of M2 alone (carbohydrates + peptone +
# yeast extract + minerals + lactate + cysteine) can support DSM 11370 in
# silico. If growth fails on M2-no-rumen, this localises the missing essential
# (most likely Co2+) to the CRF fraction of M2 — explaining how the wet-lab
# strain grows on M2 but not on FAB.
tic("[Step 9] M2-no-rumen defined-medium test")
m2_g <- NA_real_; m2_plus_co_g <- NA_real_
if (!file.exists(m2_csv)) {
  tic(sprintf("[Step 9] M2_CSV not found at %s — skipping", m2_csv))
} else {
  m2_df <- read.csv(m2_csv, stringsAsFactors = FALSE)
  tic(sprintf("[Step 9] Loaded M2_no_rumen: %d compounds", nrow(m2_df)))

  # Test 1: M2-no-rumen alone (no Co2+ supplementation)
  mod_m2 <- set_medium(mod, m2_df, close_first = TRUE)
  m2_g <- growth_of(mod_m2)
  tic(sprintf("[Step 9a] M2-no-rumen growth = %.4f h^-1   %s", m2_g,
              if (m2_g < 1e-4) "[NO growth — CRF likely supplies missing factor (Co2+?)]"
              else              "[grows on defined M2 components alone — CRF not strictly required in silico]"))

  # Test 2: M2-no-rumen + Co2+ — does adding cobalt alone rescue M2-no-rumen,
  # mirroring how it rescues FAB? If yes, this confirms Co2+ is the limiting
  # factor in M2-no-rumen and (by inference) the key missing factor that CRF
  # supplies in real M2.
  co_ex <- ex_of("cpd00149")
  if (co_ex %in% mod_m2@react_id) {
    mod_m2_co <- changeBounds(mod_m2, react = co_ex, lb = -0.02)  # 10 µM, SOP-level
    m2_plus_co_g <- growth_of(mod_m2_co)
    tic(sprintf("[Step 9b] M2-no-rumen + Co2+ (10 uM) growth = %.4f h^-1   %s",
                m2_plus_co_g,
                if (m2_plus_co_g > 1e-4 && m2_g < 1e-4)
                  "[Co2+ rescues — confirms CRF likely supplies cobalt in real M2]"
                else if (m2_plus_co_g > 1e-4 && m2_g > 1e-4)
                  "[both grow — Co2+ not limiting in M2-no-rumen]"
                else "[Co2+ does not rescue — another factor missing too]"))
  }

  # Save M2 results to TSV
  m2_results <- data.frame(
    condition   = c("M2-no-rumen (defined M2 - CRF)",
                    "M2-no-rumen + Co2+ (10 µM)"),
    growth_rate = c(m2_g, m2_plus_co_g),
    grew        = c(!is.na(m2_g) & m2_g > 1e-4,
                    !is.na(m2_plus_co_g) & m2_plus_co_g > 1e-4),
    stringsAsFactors = FALSE
  )
  write.table(m2_results, out_path("m2_norumen_results.tsv"),
              sep = "\t", quote = FALSE, row.names = FALSE)
}
...
summary_df <- rbind(
  ...
  # M2-no-rumen rows
  data.frame(condition = "M2-no-rumen (defined M2 − CRF)",
             growth_rate = m2_g, stringsAsFactors = FALSE),
  data.frame(condition = "M2-no-rumen + Co2+ (10 µM)",
             growth_rate = m2_plus_co_g, stringsAsFactors = FALSE)
)
